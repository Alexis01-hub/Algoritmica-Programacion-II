package ArchivosParcial1.MiResolucion.parcial2022.lista;
...
public class SinglyLinkedList<E> implements Cloneable {
  ...
  */ 
  public SinglyLinkedList<E> alternate(boolean odd){
    SinglyLinkedList<E> alterate = new SinglyLinkedList<>(); // lista alternada true
    Node<E> current = head; // nodo actual
    int i = 1; // contador de nodos
    while (current != null){
        if ((i % 2 == 0) != odd){ // si el mod es falso y odd es true, entonces entra al if
          alterate.addLast(current.getElement()); //agrego el elemento a la lista alternada
        }
        current = current.getNext(); // paso al siguiente nodo
        i++; // incremento el contador
    }
    return alterate; // retorno la lista alternada
  }
}
