...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  */
  public Map<Integer, List<E>> mapDepth(){
    ...
    Map<Integer, List<E>> depthMap = new UnsortedTableMap<>(); // crea un mapa para almacenar las profundidades y sus nodos
    // Iteramos sobre todas las posiciones del árbol
    for (Position<E> p : positions()) {
      int d = depth(p); // Obtenemos la profundidad del Nodo.
      List<E> lista = depthMap.get(d); // Obtenemos la lista de nodos con esa profundidad.
      if (lista == null) {
        lista = new ArrayList<>(); // Si no existe, creamos una nueva lista.
        depthMap.put(d, lista); // Agregamos la lista al mapa.
      }
      lista.add(p.getElement()); // Agregamos el elemento del nodo a la lista.
    }
    return depthMap; // Retornamos el mapa con las profundidades y sus nodos.
  }  
}
