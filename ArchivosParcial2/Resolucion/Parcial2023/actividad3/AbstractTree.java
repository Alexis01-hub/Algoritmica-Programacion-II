...
import actividad3.Map;          // for use in branchMap
import actividad3.ProbeHashMap; // for use in branchMap
...
public abstract class AbstractTree<E> implements Tree<E> {
  ...
  */
  public List<Position<E>> externals(){
    List<Position<E>> snapshot = new ArrayList<>();
    for (Position<E> p : positions()) {
      if (isExternal(p)) { // si es un nodo externo
        snapshot.add(p); // añade el nodo externo a la lista
      }
    }
    return snapshot; // devuelve la lista de nodos externos
  } 
  ...
  */
  public Map<Position<E>, List<Position<E>>> branchMap() {
    Map<Position<E>, List<Position<E>>> branch = new ProbeHashMap<>();
    for (Position<E> p : positions()) {
      if (isExternal(p)) { // si es un nodo externo
        List<Position<E>> path = new ArrayList<>(); // lista para almacenar la rama
        Position<E> current = p; // comienza desde el nodo externo
        while (current != null) { // mientras haya un nodo padre
          path.add(0, current); // añade el nodo al inicio de la lista
          current = parent(current); // sube al nodo padre
        }
        branch.put(p, path); // añade la rama al mapa con el nodo externo como clave
      }

    }
    return branch; // devuelve el mapa de ramas
  }
}
