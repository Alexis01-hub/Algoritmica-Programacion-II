...
public Iterable<Position<E>> breadthfirst() {
  List<Position<E>> snapshot = new ArrayList<>();
  if (!isEmpty()) {
    java.util.Stack<Position<E>> stack = new java.util.Stack<>(); // Usamos una pila para el recorrido
    stack.push(root()); // Comenzamos desde la raíz
    while (!stack.isEmpty()) { // Mientras haya posiciones en la pila
      Position<E> p = stack.pop(); // Sacamos la posición del tope de la pila
      snapshot.add(p); // Agregamos la posición al snapshot
      // Para mantener el orden de izquierda a derecha, agregamos los hijos de derecha a izquierda
      List<Position<E>> childrenList = new ArrayList<>(); // Usamos una lista para almacenar los hijos
      for (Position<E> c : children(p)) // Iteramos sobre los hijos de la posición actual
        childrenList.add(c); //
      for (int i = childrenList.size() - 1; i >= 0; i--) // Agregamos los hijos a la pila en orden inverso
        stack.push(childrenList.get(i));
    }
  }
  return snapshot; // devolvemos el snapshot con las posiciones en orden de recorrido por niveles
}
